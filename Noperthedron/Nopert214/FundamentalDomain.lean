...
noncomputable def _root_.MatrixPose.rightNopert214Symmetry
    (p : MatrixPose) (k : OrbitIndex) : MatrixPose where
  innerRot := p.innerRot * fivefoldSO3 k
  outerRot := p.outerRot
  innerOffset := p.innerOffset

theorem innerShadow_rightNopert214Symmetry
    (p : MatrixPose) (k : OrbitIndex) :
    innerShadow (p.rightNopert214Symmetry k) exactPolyhedron.hull =
  ...
  · rintro ⟨v, hv, rfl⟩
    ...
    simp [MatrixPose.inner_apply, MatrixPose.rightNopert214Symmetry,
...
@[simp] theorem outerShadow_rightNopert214Symmetry
    (p : MatrixPose) (k : OrbitIndex) :
    outerShadow (p.rightNopert214Symmetry k) exactPolyhedron.hull =
      outerShadow p exactPolyhedron.hull := by
  rfl

theorem RupertPose_rightNopert214Symmetry_iff
    (p : MatrixPose) (k : OrbitIndex) :
    RupertPose (p.rightNopert214Symmetry k) exactPolyhedron.hull ↔
      RupertPose p exactPolyhedron.hull := by
  simp only [RupertPose, innerShadow_rightNopert214Symmetry,
    outerShadow_rightNopert214Symmetry]
...
def _root_.MatrixPose.InNopert214FundamentalDomain
    (p : MatrixPose) : Prop :=
  InFivefoldFundamentalDomain p.relativeRotation

@[simp] theorem MatrixPose.relativeRotation_rightNopert214Symmetry
    (p : MatrixPose) (k : OrbitIndex) :
    (p.rightNopert214Symmetry k).relativeRotation =
      p.relativeRotation * fivefoldMatrix k := by
  simp [MatrixPose.relativeRotation,
    MatrixPose.rightNopert214Symmetry, fivefoldSO3, Matrix.mul_assoc]

theorem MatrixPose.exists_rightNopert214Symmetry_inFundamentalDomain
    (p : MatrixPose) :
    ∃ k : OrbitIndex,
      (p.rightNopert214Symmetry k).InNopert214FundamentalDomain := by
  obtain ⟨k, hk⟩ := exists_mul_fivefold_inFundamentalDomain p.relativeRotation
  exact ⟨k, by simpa [MatrixPose.InNopert214FundamentalDomain] using hk⟩
