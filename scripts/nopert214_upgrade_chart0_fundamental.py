#!/usr/bin/env python3
"""Replace chart-0 split subtrees by exact fundamental-domain prune leaves."""
...
from nopert214_certificate_search import (
    Q,
    atlas_fundamental_outside_float,
    atlas_fundamental_status,
)


def upgrade(data):
    if int(data.get("chart", -1)) != 0:
        raise ValueError("fundamental migration requires chart 0")
    rows = data["rows"]
    ...
    def revisit(row_id):
        nonlocal audits, exact_rejections
        if row_id in visited:
            raise ValueError(f"tree row {row_id} is reachable more than once")
        visited.add(row_id)
        row = rows[row_id]
        if (row is not None and row.get("kind") not in
                ("view_root", "view_split", "relative_split")):
            return
        node = geometry(row_id)
        center = tuple(map(Q, node["center"]))
        widths = tuple(map(Q, node["widths"]))
        if atlas_fundamental_outside_float(0, center, widths) is not None:
            audits += 1
            status, direction, _ = atlas_fundamental_status(
                0, center, widths)
            if status == "outside":
                replacements[row_id] = {
                    "id": row_id,
                    "kind": "fundamental_prune",
                    "center": node["center"],
                    "widths": node["widths"],
                    "root": node["root"],
    ...
    data["counts"]["fundamental_audit"] = (
        data["counts"].get("fundamental_audit", 0) + audits)
    return {
        "audits": audits,
